...
注册登录、角色权限、PostgreSQL 持久化、AI 权限契约、前端角色化界面、登录后恢复最近十轮聊天历史、API/架构文档更新均已完成。已执行 Java 后端测试、AI 服务测试和前端构建。
...
- 普通用户不查看文档列表，只使用聊天问答。
- 用户登录后默认加载最近会话的最新 10 轮问答。
- AI 服务查询权限以 `knowledge_base_ids` 为主，`allowed_user_ids` 仅保留兼容。

## 命令记录

- `.\mvnw.cmd test`：通过，`ApiControllerTest` 9 个测试通过。
- `D:\anaconda3\envs\rag-ai\python.exe -m unittest discover -s tests`：通过，10 个测试通过。
...
- 当前测试以控制器/服务逻辑和 AI 单元测试为主，尚未引入真实 PostgreSQL 或 Testcontainers 集成测试。
- 若本机已有旧 PostgreSQL volume，Java 启动时会自动补业务表和列；pgvector 扩展和 `document_chunks` 向量表仍依赖 `deploy/postgres/init.sql` 或现有部署初始化。
